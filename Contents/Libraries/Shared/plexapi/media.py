...
class Media(PlexObject):
    ...
    def delete(self):
        part = '{}/media/{}'.format((self._parentKey), (self.id))
        try:
...
class MediaPart(PlexObject):
    ...
    def setSelectedAudioStream(self, stream):
        ...
        """
        key = '/library/parts/{}'.format((self.id))
        params = {'allParts': 1}
...
class Conversion(PlexObject):
    ...
    def remove(self):
        ...
        key = '/playlists/{}/items/{}/{}/disable'.format((self.playlistID), (self.generatorID), (self.ratingKey))
        self._server.query(key, method=self._server._session.put)
    ...
    def move(self, after):
        ...
        key = '{}/items/{}/move?after={}'.format((self._initpath), (self.playQueueItemID), (after))
        self._server.query(key, method=self._server._session.put)
...
class MediaTag(PlexObject):
    ...
    def _loadData(self, data):
        ...
        if self._librarySectionKey and self.filter:
            self.key = '{}/all?{}&type={}'.format((self._librarySectionKey), (self.filter),
                                                  (utils.searchType(self._parentType)))

    def items(self):
        """ Return the list of items within this tag. """
        if not self.key:
            raise BadRequest('Key is not defined for this tag: {}. Reload the parent object.'.format((self.tag)))
...
class Collection(MediaTag):
    ...
    def collection(self):
        ...
        """
        key = '{}/collections'.format((self._librarySectionKey))
...
class BaseResource(PlexObject):
    ...
    def select(self):
        ...
        data = '{}?url={}'.format((key), (quote_plus(self.ratingKey)))
        try:
...
class Chapter(PlexObject):
    ...
    def __repr__(self):
        ...
        return "<{}>".format((':'.join([self.__class__.__name__, name, offsets])))
...
class SearchResult(PlexObject):
    ...
    def __repr__(self):
        ...
        return "<{}>".format((':'.join([p for p in [self.__class__.__name__, name, score] if p])))
...
class Agent(PlexObject):
    ...
    def __repr__(self):
        ...
        return "<{}>".format((':'.join([p for p in [self.__class__.__name__, uid] if p])))
    ...
    def settings(self):
        key = '/:/plugins/{}/prefs'.format((self.identifier))
...
class Availability(PlexObject):
    ...
    def __repr__(self):
        return '<{}:{}:{}>'.format((self.__class__.__name__), (self.platform), (self.offerType))
