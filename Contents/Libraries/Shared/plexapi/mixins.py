...
class AdvancedSettingsMixin(object):
    ...
    def preference(self, pref):
        ...
        try:
            ...
            raise NotFound('Unknown preference "{}" for {}. '
                           'Available preferences: {}'.format((pref), (self.TYPE), (availablePrefs)))

    def editAdvanced(self, **kwargs):
        """ Edit a Plex object's advanced settings. """
        data = {}
        key = '{}/prefs?'.format((self.key))
        ...
        for settingID, value in kwargs.items():
            try:
                ...
                raise NotFound('{} not found in {}'.format((value), (list(preferences.keys()))))

            enumValues = pref.enumValues
            if enumValues.get(value, enumValues.get(str(value))):
                data[settingID] = value
            else:
                raise NotFound('{} not found in {}'.format((value), (list(enumValues))))
...
class SplitMergeMixin(object):
    ...
    def merge(self, ratingKeys):
        ...
        key = "{}/merge?ids={}".format((self.key), (','.join(str(r) for r in ratingKeys)))
        self._server.query(key, method=self._server._session.put)
        return self
    ...
    def fixMatch(self, searchResult=None, auto=False, agent=None):
        """ Use match result to update show metadata.
            ...
            Parameters:
                ...
                agent (str): Agent name to be used (imdb, thetvdb, themoviedb, etc.)
        """
        key = '{}/match'.format((self.key))
        if auto:
            autoMatch = self.matches(agent=agent)
            if autoMatch:
                searchResult = autoMatch[0]
            else:
                raise NotFound('No matches found using this agent: ({}:{})'.format((agent), (autoMatch)))
...
class HubsMixin(object):
    ...
    def hubs(self):
        ...
        key = '{}/related'.format((self.key))
        data = self._server.query(key)
        return self.findItems(data, Hub)


class PlayedUnplayedMixin(object):
...
class RatingMixin(object):
    ...
    def rate(self, rating=None):
        ...
        key = '/:/rate?key={}&identifier=com.plexapp.plugins.library&rating={}'.format((self.ratingKey), (rating))
        self._server.query(key, method=self._server._session.put)
        return self
...
class ArtMixin(ArtUrlMixin, ArtLockMixin):
    ...
    def arts(self):
        ...
        return self.fetchItems('/library/metadata/{}/arts'.format((self.ratingKey)), cls=media.Art)
    ...
    def uploadArt(self, url=None, filepath=None):
        ...
        """
        if url:
            key = '/library/metadata/{}/arts?url={}'.format((self.ratingKey), (quote_plus(url)))
            self._server.query(key, method=self._server._session.post)
        elif filepath:
            key = '/library/metadata/{}/arts'.format((self.ratingKey))
...
class PosterMixin(PosterUrlMixin, PosterLockMixin):
    ...
    def posters(self):
        ...
        return self.fetchItems('/library/metadata/{}/posters'.format((self.ratingKey)), cls=media.Poster)
    ...
    def uploadPoster(self, url=None, filepath=None):
        ...
        """
        if url:
            key = '/library/metadata/{}/posters?url={}'.format((self.ratingKey), (quote_plus(url)))
            self._server.query(key, method=self._server._session.post)
        elif filepath:
            key = '/library/metadata/{}/posters'.format((self.ratingKey))
            data = openOrRead(filepath)
            self._server.query(key, method=self._server._session.post, data=data)
        return self
...
class ThemeMixin(ThemeUrlMixin, ThemeLockMixin):
    ...
    def themes(self):
        ...
        return self.fetchItems('/library/metadata/{}/themes'.format((self.ratingKey)), cls=media.Theme)
    ...
    def uploadTheme(self, url=None, filepath=None, timeout=None):
        ...
        """
        if url:
            key = '/library/metadata/{}/themes?url={}'.format((self.ratingKey), (quote_plus(url)))
            self._server.query(key, method=self._server._session.post, timeout=timeout)
        elif filepath:
            key = '/library/metadata/{}/themes'.format((self.ratingKey))
            data = openOrRead(filepath)
...
class EditFieldMixin(object):
    ...
    def editField(self, field, value, locked=True, **kwargs):
        ...
        """
        edits = {
            '{}.value'.format((field)): value or '',
            '{}.locked'.format((field)): 1 if locked else 0
        }
        edits.update(kwargs)
...
class EditTagsMixin(object):
    ...
    def _tagHelper(tag, items, locked=True, remove=False):
        ...
        data = {
            '{}.locked'.format((tag)): 1 if locked else 0
        }

        if remove:
            tagname = '{}[].tag.tag-'.format((tag))
            data[tagname] = ','.join(quote(str(t)) for t in items)
        else:
            for i, item in enumerate(items):
                tagname = '{}[{}].tag.tag'.format((str(tag)), (i))
                data[tagname] = item
...
class WatchlistMixin(object):
    ...
    def streamingServices(self, account=None):
        ...
        data = account.query("{}/library/metadata/{}/availabilities".format((account.METADATA), (ratingKey)))
