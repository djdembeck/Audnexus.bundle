...
class Artist(
    ...
    def station(self):
        ...
        key = '{}?includeStations=1'.format((self.key))
    ...
    def metadataDirectory(self):
        ...
        return str(Path('Metadata') / 'Artists' / guid_hash[0] / '{}.bundle'.format((guid_hash[1:])))
...
class Album(
    ...
    def metadataDirectory(self):
        """ Returns the Plex Media Server data directory where the metadata is stored. """
        guid_hash = utils.sha1hash(self.guid)
        return str(Path('Metadata') / 'Albums' / guid_hash[0] / '{}.bundle'.format((guid_hash[1:])))
...
class Track(
    ...
    def _prettyfilename(self):
        ...
        return '{} - {} - {} - {}'.format((self.grandparentTitle), (self.parentTitle), (str(self.trackNumber).zfill(2)), (self.title))
    ...
    def _defaultSyncTitle(self):
        """ Returns str, default title for a new syncItem. """
        return '{} - {} - {}'.format((self.grandparentTitle), (self.parentTitle), (self.title))
