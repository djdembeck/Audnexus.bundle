...
from plexapi.backports import unicodedata_normalize
...
def registerPlexObject(cls):
    ...
    ehash = '{}.{}'.format((cls.TAG), (etype)) if etype else cls.TAG
    if getattr(cls, '_SESSIONTYPE', None):
        ehash = "{}.session".format((ehash))
    elif getattr(cls, '_HISTORYTYPE', None):
        ehash = "{}.history".format((ehash))
    if ehash in PLEXOBJECTS:
        raise Exception('Ambiguous PlexObject definition {}(tag={}, type={}) '
                        'with {}'.format((cls.__name__), (cls.TAG), (etype), (PLEXOBJECTS[ehash].__name__)))
...
def joinArgs(args):
    ...
    for key in sorted(args, key=lambda x: x.lower()):
        value = str(args[key])
        arglist.append("{}={}".format((key), (quote(value, safe=''))))
    return "?{}".format(('&'.join(arglist)))
...
def searchType(libtype):
    ...
    try:
        ...
        if libtype in [str(k) for k in REVERSESEARCHTYPES]:
            return libtype
        raise NotFound('Unknown libtype: {}'.format((libtype)))
...
def tagType(tag):
    ...
    try:
        ...
        if tag in [str(k) for k in REVERSETAGTYPES]:
            return tag
        raise NotFound('Unknown tag: {}'.format((tag)))
...
def millisecondToHumanstr(milliseconds):
    ...
    return ('' if days == 0 else '{} day{}, '.format((days), ("s" if days > 1 else ""))) + '{:02d}:{:02d}:{:02d}.{:03d}'.format((hours), (mins), (secs), (ms))
...
def cleanFilename(filename, replace='_'):
    ...
    cleaned_filename = unicodedata_normalize('NFKD', filename).encode('ASCII', 'ignore').decode()
    ...
    for media in server.sessions():
        ...
        for part in media.iterParts():
            ...
            if part.indexes:  # always use bif images if available.
                url = '/library/parts/{}/indexes/{}/{}'.format((part.id), (part.indexes.lower()), (media.viewOffset))
        if url:
            if filename is None:
                prettyname = media._prettyfilename()
                filename = 'session_transcode_{}_{}_{}'.format((media.usernames[0]), (prettyname), (int(time.time())))
...
def getMyPlexAccount(opts=None):  # pragma: no cover
    ...
    if opts and opts.username and opts.password:
        print('Authenticating with Plex.tv as {}..'.format((opts.username)))
    ...
    if config_username and config_password:
        print('Authenticating with Plex.tv as {}..'.format((config_username)))
    ...
    print('Authenticating with Plex.tv as {}..'.format((username)))
...
def getAgentIdentifier(section, agent):
    ...
    for ag in section.agents():
        ...
        agents += identifiers
    raise NotFound("Could not find \"{}\" in agents list ({})".format((agent), (', '.join(agents))))
...
def deprecated(message, stacklevel=2):
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            msg = 'Call to deprecated function or method "{}", {}.'.format((func.__name__), (message))
