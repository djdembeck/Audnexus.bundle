...
class SyncItem(PlexObject):
    ...
    def server(self):
        ...
        if len(server) == 0:
            raise NotFound('Unable to find server with uuid {}'.format((self.machineIdentifier)))
        return server[0]

    def getMedia(self):
        """ Returns list of :class:`~plexapi.base.Playable` which belong to this sync item. """
        server = self.server().connect()
        key = '/sync/items/{}'.format((self.id))
    ...
    def markDownloaded(self, media):
        ...
        """
        url = '/sync/{}/item/{}/downloaded'.format((self.clientIdentifier), (media.ratingKey))
