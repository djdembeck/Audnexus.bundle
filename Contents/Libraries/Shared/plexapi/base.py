...
class PlexObject(object):
    ...
    def __repr__(self):
        ...
        return "<{}>".format((':'.join([p for p in [self.__class__.__name__, uid, name] if p])))
    ...
    def _buildItem(self, elem, cls=None, initpath=None):
        ...
        ehash = '{}.{}'.format((elem.tag), (etype)) if etype else elem.tag
    ...
    def fetchItems(self, ekey, cls=None, container_start=None, container_size=None, maxresults=None, **kwargs):
        ...
        if isinstance(ekey, list) and all(isinstance(key, int) for key in ekey):
            ekey = '/library/metadata/{}'.format((",".join(str(key) for key in ekey)))
    ...
    def fetchItem(self, ekey, cls=None, **kwargs):
        ...
        if isinstance(ekey, int):
            ekey = '/library/metadata/{}'.format((ekey))

        try:
            return self.fetchItems(ekey, cls, **kwargs)[0]
        except IndexError:
            clsname = cls.__name__ if cls else 'None'
            raise NotFound('Unable to find elem: cls={}, attrs={}'.format((clsname), (kwargs)))
    ...
    def listAttrs(self, data, attr, rtag=None, **kwargs):
        ...
        if rtag:
            data = next(utils.iterXMLBFS(data, rtag), [])
        for elem in data:
            kwargs['{}__exists'.format((attr))] = True
            if self._checkAttrs(elem, **kwargs):
...
class PlexPartialObject(PlexObject):
    ...
    def __getattribute__(self, attr):
        ...
        objname = "{} '{}'".format((clsname), (title)) if title else clsname
    ...
    def analyze(self):
        ...
        """
        key = "/{}/analyze".format((self.key.lstrip('/')))
...
class Playable(object):
    ...
    def getStreamURL(self, **kwargs):
        ...
        if self.TYPE not in ('movie', 'episode', 'track', 'clip'):
            raise Unsupported('Fetching stream URL for {} is unsupported.'.format((self.TYPE)))
        ...
        return self._server.url(
            '/{}/:/transcode/universal/start.{}?{}'.format((streamtype), (ext), (urlencode(params))),
            includeToken=True
        )
    ...
    def download(self, savepath=None, keep_original_name=False, **kwargs):
        ...
        for part in parts:
            if not keep_original_name:
                filename = utils.cleanFilename('{}.{}'.format((self._prettyfilename()), (part.container)))
            else:
                filename = part.file
            ...
            if kwargs:
                ...
                download_url = self._server.url('{}?download=1'.format((part.key)))
    ...
    def updateProgress(self, time, state='stopped'):
        ...
        """
        key = '/:/progress?key={}&identifier=com.plexapp.plugins.library&time={}&state={}'.format(
            (self.ratingKey), (time), (state))
    ...
    def updateTimeline(self, time, state='stopped', duration=None):
        ...
        key = ('/:/timeline?ratingKey={}&key={}&'
               'identifier=com.plexapp.plugins.library&time={}&state={}{}'.format((self.ratingKey), (self.key), (int(time)), (state), (durationStr)))
        self._server.query(key)
        return self
