...
class MyPlexAccount(PlexObject):
    ...
    def device(self, name=None, clientId=None):
        ...
        for device in self.devices():
            if (name and device.name.lower() == name.lower() or device.clientIdentifier == clientId):
                return device
        raise NotFound('Unable to find device {}'.format((name)))
    ...
    def resource(self, name):
        ...
        for resource in self.resources():
            if resource.name.lower() == name.lower():
                return resource
        raise NotFound('Unable to find resource {}'.format((name)))
    ...
    def switchHomeUser(self, user, pin=None):
        ...
        """
        user = user if isinstance(user, MyPlexUser) else self.user(user)
        url = '{}/{}/switch'.format((self.HOMEUSERS), (user.id))
        params = {}
        if pin:
            params['pin'] = pin
    ...
    def acceptInvite(self, user):
        ...
        url = MyPlexInvite.REQUESTS + '/{}'.format((invite.id)) + utils.joinArgs(params)
    ...
    def cancelInvite(self, user):
        ...
        url = MyPlexInvite.REQUESTED + '/{}'.format((invite.id)) + utils.joinArgs(params)
        return self.query(url, self._session.delete)
    ...
    def user(self, username):
        ...
        raise NotFound('Unable to find user {}'.format((username)))
    ...
    def pendingInvite(self, username, includeSent=True, includeReceived=True):
        ...
        raise NotFound('Unable to find invite {}'.format((username)))
    ...
    def _filterDictToStr(self, filterDict):
        ...
        for key, vals in filterDict.items():
            if key not in ('contentRating', 'label', 'contentRating!', 'label!'):
                raise BadRequest('Unknown filter key: {}'.format((key)))
            values.append("{}={}".format((key), ('%2C'.join(vals))))
        return '|'.join(values)
    ...
    def deleteWebhook(self, url):
        ...
        if url not in urls:
            raise BadRequest('Webhook does not exist: {}'.format((url)))
        urls.remove(url)
        return self.setWebhooks(urls)
    ...
    def sync(self, sync_item, client=None, clientId=None):
        ...
        if not client:
            ...
            if not client:
                raise BadRequest('Unable to find client by clientId={}'.format((clientId)))
    ...
    def claimToken(self):
        ...
        if response.status_code not in (200, 201, 204):  # pragma: no cover
            codename = codes.get(response.status_code)[0]
            errtext = response.text.replace('\n', ' ')
            raise BadRequest('({}) {} {}; {}'.format((response.status_code), (codename), (response.url), (errtext)))
    ...
    def videoOnDemand(self):
        ...
        """
        data = self.query('{}/hubs'.format((self.VOD)))
        return self.findItems(data)

    def tidal(self):
        """ Returns a list of tidal Hub items :class:`~plexapi.library.Hub`
        """
        data = self.query('{}/hubs'.format((self.MUSIC)))
        return self.findItems(data)
    ...
    def watchlist(self, filter=None, sort=None, libtype=None, maxresults=None, **kwargs):
        ...
        key = '{}/library/sections/watchlist/{}{}'.format((self.METADATA), (filter), (utils.joinArgs(params)))
    ...
    def addToWatchlist(self, items):
        ...
        for item in items:
            if self.onWatchlist(item):
                raise BadRequest('"{}" is already on the watchlist'.format((item.title)))
            ratingKey = item.guid.rsplit('/', 1)[-1]
            self.query('{}/actions/addToWatchlist?ratingKey={}'.format((self.METADATA), (ratingKey)), method=self._session.put)
        return self
    ...
    def removeFromWatchlist(self, items):
        ...
        for item in items:
            if not self.onWatchlist(item):
                raise BadRequest('"{}" is not on the watchlist'.format((item.title)))
            ratingKey = item.guid.rsplit('/', 1)[-1]
            self.query('{}/actions/removeFromWatchlist?ratingKey={}'.format((self.METADATA),
                       (ratingKey)), method=self._session.put)
        return self
    ...
    def userState(self, item):
        ...
        """
        ratingKey = item.guid.rsplit('/', 1)[-1]
        data = self.query("{}/library/metadata/{}/userState".format((self.METADATA), (ratingKey)))
    ...
    def markPlayed(self, item):
        """ Mark the Plex object as played on Discover.
            ...
            Parameters:
                ...
                Can be also result from Plex Movie or Plex TV Series agent.
        """
        key = '{}/actions/scrobble'.format((self.METADATA))
        ratingKey = item.guid.rsplit('/', 1)[-1]
    ...
    def markUnplayed(self, item):
        """ Mark the Plex object as unplayed on Discover.
            ...
            Parameters:
                ...
                Can be also result from Plex Movie or Plex TV Series agent.
        """
        key = '{}/actions/unscrobble'.format((self.METADATA))
        ratingKey = item.guid.rsplit('/', 1)[-1]
        params = {'key': ratingKey, 'identifier': 'com.plexapp.plugins.library'}
        self.query(key, params=params)
        return self
    ...
    def searchDiscover(self, query, limit=30, libtype=None):
        ...
        for result in searchResult:
            ...
            if sys.version_info[0] < 3:
                ...
                attrs = ''.join('{}="{}" '.format((k), (html.escape(str(v)))) for k, v in metadata.items())
            xml = '<{} {}/>'.format((tag), (attrs))
...
class MyPlexUser(PlexObject):
    ...
    def server(self, name):
        ...
        raise NotFound('Unable to find server {}'.format((name)))
...
class MyPlexServerShare(PlexObject):
    ...
    def section(self, name):
        ...
        raise NotFound('Unable to find section {}'.format((name)))
...
class ResourceConnection(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.httpuri = 'http://{}:{}'.format((self.address), (self.port))
...
class MyPlexDevice(PlexObject):
    ...
    def delete(self):
        ...
        key = 'https://plex.tv/devices/{}.xml'.format((self.id))
...
class MyPlexPinLogin(object):
    ...
    def oauthUrl(self, forwardUrl=None):
        ...
        return 'https://app.plex.tv/auth/#!?{}'.format((urlencode(params)))
...
def _chooseConnection(ctype, name, results):
    ...
    if results:
        ...
        return results[0]
    raise NotFound('Unable to connect to {}: {}'.format((ctype.lower()), (name)))
...
class AccountOptOut(PlexObject):
    ...
    def _updateOptOut(self, option):
        ...
        if option not in self.CHOICES:
            raise NotFound('{} not found in available choices: {}'.format((option), (self.CHOICES)))
    ...
    def optOutManaged(self):
        ...
        if self.key == 'tv.plex.provider.music':
            raise BadRequest('{} does not have the option to opt out managed users.'.format((self.key)))
...
class UserState(PlexObject):
    ...
    def __repr__(self):
        return '<{}:{}>'.format((self.__class__.__name__), (self.ratingKey))
