...
class Video(PlexPartialObject, PlayedUnplayedMixin):
    ...
    def searchSubtitles(self, language='en', hearingImpaired=0, forced=0):
        ...
        key = '{}/subtitles{}'.format((self.key), (utils.joinArgs(params)))
    ...
    def removeSubtitles(self, subtitleStream=None, streamID=None, streamTitle=None):
        ...
        if subtitleStream is None:
            try:
                ...
                raise BadRequest("Subtitle stream with ID '{}' or title '{}' not found.".format((streamID), (streamTitle)))
    ...
    def optimize(self, title='', target='', deviceProfile='', videoQuality=None,
        ...
        key = '{}/items'.format((backgroundProcessing.key))
        ...
        if targetTagID:
            ...
            target = 'Custom: {}'.format((deviceProfile))
        ...
        if locationID not in libraryLocationIDs:
            raise BadRequest('Unknown location ID "{}" not in {}'.format((locationID), (libraryLocationIDs)))

        if isinstance(self, (Show, Season)):
            uri = 'library:///directory/{}'.format((quote_plus("{}/children".format((self.key)))))
        else:
            uri = 'library://{}/item/{}'.format((section.uuid), (quote_plus(self.key)))
    ...
    def sync(self, videoQuality, client=None, clientId=None, limit=None, unwatched=False, title=None):
        ...
        sync_item.location = 'library://{}/item/{}'.format((section.uuid), (quote_plus(self.key)))
...
class Movie(
    ...
    def metadataDirectory(self):
        ...
        return str(Path('Metadata') / 'Movies' / guid_hash[0] / '{}.bundle'.format((guid_hash[1:])))
...
class Show(
    ...
    def season(self, title=None, season=None):
        ...
        """
        key = '{}/children?excludeAllLeaves=1'.format((self.key))
    ...
    def download(self, savepath=None, keep_original_name=False, subfolders=False, **kwargs):
        ...
        for episode in self.episodes():
            _savepath = os.path.join(savepath, 'Season {}'.format(
                (str(episode.seasonNumber).zfill(2)))) if subfolders else savepath
    ...
    def metadataDirectory(self):
        """ Returns the Plex Media Server data directory where the metadata is stored. """
        guid_hash = utils.sha1hash(self.guid)
        return str(Path('Metadata') / 'TV Shows' / guid_hash[0] / '{}.bundle'.format((guid_hash[1:])))
...
class Season(
    ...
    def __repr__(self):
        return '<{}>'.format(
            ':'.join([p for p in [
                ...
                "{}-{}".format((self.parentTitle.replace(' ', '-')[:20]), (self.seasonNumber)),
            ] if p])
        )
    ...
    def _defaultSyncTitle(self):
        """ Returns str, default title for a new syncItem. """
        return '{} - {}'.format((self.parentTitle), (self.title))
...
class Episode(
    ...
    def parentKey(self):
        ...
        if self.parentRatingKey:
            return '/library/metadata/{}'.format((self.parentRatingKey))
        return None
    ...
    def _season(self):
        ...
        if self.grandparentKey and self.parentIndex is not None:
            return self.fetchItem('{}/children?excludeAllLeaves=1&index={}'.format((self.grandparentKey), (self.parentIndex)))
        return None
    ...
    def __repr__(self):
        return '<{}>'.format(
            ':'.join([p for p in [
                self.__class__.__name__,
                self.key.replace('/library/metadata/', '').replace('/children', ''),
                "{}-{}".format((self.grandparentTitle.replace(' ', '-')[:20]), (self.seasonEpisode)),
            ] if p])
        )

    def _prettyfilename(self):
        """ Returns a filename for use in download. """
        return '{} - {} - {}'.format((self.grandparentTitle), (self.seasonEpisode), (self.title))
    ...
    def seasonEpisode(self):
        ...
        return 's{}e{}'.format((str(self.seasonNumber).zfill(2)), (str(self.episodeNumber).zfill(2)))
    ...
    def _defaultSyncTitle(self):
        """ Returns str, default title for a new syncItem. """
        return '{} - {} - ({}) {}'.format((self.grandparentTitle), (self.parentTitle), (self.seasonEpisode), (self.title))
...
class Extra(Clip):
    ...
    def _prettyfilename(self):
        """ Returns a filename for use in download. """
        return '{} ({})'.format((self.title), (self.subtype))
