...
class Collection(
    ...
    def item(self, title):
        ...
        for item in self.items():
            if item.title.lower() == title.lower():
                return item
        raise NotFound('Item with title "{}" not found in the collection'.format((title)))
    ...
    def visibility(self):
        ...
        key = '/hubs/sections/{}/manage?metadataItemId={}'.format((self.librarySectionID), (self.ratingKey))
        data = self._server.query(key)
        hub = self.findItem(data, cls=ManagedHub)
        if hub is None:
            hub = ManagedHub(self._server, data, parent=self)
            hub.identifier = 'custom.collection.{}.{}'.format((self.librarySectionID), (self.ratingKey))
    ...
    def filterUserUpdate(self, user=None):
        ...
        if key is None:
            raise BadRequest('Unknown collection filtering user: {}. Options {}'.format((user), (list(user_dict))))
    ...
    def modeUpdate(self, mode=None):
        ...
        if key is None:
            raise BadRequest('Unknown collection mode: {}. Options {}'.format((mode), (list(mode_dict))))
    ...
    def sortUpdate(self, sort=None):
        ...
        if key is None:
            raise BadRequest('Unknown sort dir: {}. Options: {}'.format((sort), (list(sort_dict))))
    ...
    def addItems(self, items):
        ...
        for item in items:
            if item.type != self.subtype:  # pragma: no cover
                raise BadRequest('Can not mix media types when building a collection: {} and {}'.format(
                    (self.subtype), (item.type)))
            ratingKeys.append(str(item.ratingKey))

        ratingKeys = ','.join(ratingKeys)
        uri = '{}/library/metadata/{}'.format((self._server._uriRoot()), (ratingKeys))

        args = {'uri': uri}
        key = "{}/items{}".format((self.key), (utils.joinArgs(args)))
    ...
    def removeItems(self, items):
        ...
        for item in items:
            key = '{}/items/{}'.format((self.key), (item.ratingKey))
    ...
    def moveItem(self, item, after=None):
        ...
        key = '{}/items/{}/move'.format((self.key), (item.ratingKey))

        if after:
            key += '?after={}'.format((after.ratingKey))
    ...
    def updateFilters(self, libtype=None, limit=None, sort=None, filters=None, **kwargs):
        ...
        uri = '{}{}'.format((self._server._uriRoot()), (searchKey))
    ...
    def _create(cls, server, title, section, items):
        ...
        ratingKeys = ','.join(ratingKeys)
        uri = '{}/library/metadata/{}'.format((server._uriRoot()), (ratingKeys))

        args = {'uri': uri, 'type': utils.searchType(itemType), 'title': title, 'smart': 0, 'sectionId': section.key}
        key = "/library/collections{}".format((utils.joinArgs(args)))
    ...
    def _createSmart(cls, server, title, section, limit=None, libtype=None, sort=None, filters=None, **kwargs):
        ...
        searchKey = section._buildSearchKey(
            sort=sort, libtype=libtype, limit=limit, filters=filters, **kwargs)
        uri = '{}{}'.format((server._uriRoot()), (searchKey))
    ...
    def sync(self, videoQuality=None, photoResolution=None, audioBitrate=None, client=None, clientId=None, limit=None,
        ...
        key = quote_plus('{}/children?excludeAllLeaves=1'.format((self.key)))
        sync_item.location = 'library:///directory/{}'.format((key))
