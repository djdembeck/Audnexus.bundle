...
class Settings(PlexObject):
    ...
    def get(self, id):
        ...
        raise NotFound('Invalid setting id: {}'.format((id)))
    ...
    def save(self):
        ...
        querystr = '&'.join('{}={}'.format((k), (v)) for k, v in params.items())
        url = '{}?{}'.format((self.key), (querystr))
...
class Setting(PlexObject):
    ...
    def set(self, value):
        ...
        if not isinstance(value, self.TYPES[self.type]['type']):
            ...
            raise BadRequest('Invalid value for {}: a {} is required, not {}'.format((self.id), (self.type), (badtype)))
        if self.enumValues and value not in self.enumValues:
            raise BadRequest('Invalid value for {}: {} not in {}'.format((self.id), (value), (list(self.enumValues))))
    ...
    def toUrl(self):
        """Helper for urls"""
        return '{}={}'.format((self.id), (self._value or self.value))
...
class Preferences(Setting):
    ...
    def _default(self):
        ...
        key = '{}/prefs?'.format((self._initpath))
        url = key + '{}={}'.format((self.id), (self.default))
