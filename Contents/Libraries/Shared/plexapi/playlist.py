...
class Playlist(
    ...
    def _getPlaylistItemID(self, item):
        ...
        raise NotFound('Item with title "{}" not found in the playlist'.format((item.title)))
    ...
    def item(self, title):
        ...
        for item in self.items():
            if item.title.lower() == title.lower():
                return item
        raise NotFound('Item with title "{}" not found in the playlist'.format((title)))
        ...
        for item in items:
            if item.listType != self.playlistType:  # pragma: no cover
                raise BadRequest('Can not mix media types when building a playlist: '
                                 '{} and {}'.format((self.playlistType), (item.listType)))
    ...
    def removeItems(self, items):
        ...
        for item in items:
            ...
            key = '{}/items/{}'.format((self.key), (playlistItemID))
    ...
    def moveItem(self, item, after=None):
        ...
        playlistItemID = self._getPlaylistItemID(item)
        key = '{}/items/{}/move'.format((self.key), (playlistItemID))

        if after:
            afterPlaylistItemID = self._getPlaylistItemID(after)
            key += '?after={}'.format((afterPlaylistItemID))
    ...
    def _edit(self, **kwargs):
        ...
        key = '{}{}'.format((self.key), (utils.joinArgs(kwargs)))
        self._server.query(key, method=self._server._session.put)
        return self
    ...
    def _createFromM3U(cls, server, title, section, m3ufilepath):
        ...
        key = "/playlists/upload{}".format((utils.joinArgs(args)))
        ...
        try:
            ...
            raise BadRequest('Failed to create playlist from m3u file.')
    ...
    def sync(self, videoQuality=None, photoResolution=None, audioBitrate=None, client=None, clientId=None, limit=None,
        ...
        sync_item.location = 'playlist:///{}'.format((quote_plus(self.guid)))
