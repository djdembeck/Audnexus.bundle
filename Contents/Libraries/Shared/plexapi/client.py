...
class PlexClient(PlexObject):
    ...
    def connect(self, timeout=None):
        ...
        if self._clientIdentifier:
            ...
            if client is None:
                raise NotFound(
                    "Client with identifier {} not found at {}".format((self._clientIdentifier), (self._baseurl))
                )
        else:
            client = data[0]
    ...
    def sendCommand(self, command, proxy=None, **params):
        ...
        key = '/player/{}{}'.format((command), (utils.joinArgs(params)))
    ...
    def playMedia(self, media, offset=0, **params):
        ...
        command = {
            ...
            'type': mediatype,
            'containerKey': '/playQueues/{}?window=100&own=1'.format((playqueue.playQueueID)),
        }
        command.update(params)
        token = media._server.createToken()
        if token:
