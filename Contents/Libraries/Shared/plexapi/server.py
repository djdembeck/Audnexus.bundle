...
class PlexServer(PlexObject):
    ...
    def _uriRoot(self):
        return 'server://{}/com.plexapp.plugins.library'.format((self.machineIdentifier))
    ...
    def agents(self, mediaType=None):
        ...
        key = '/system/agents'
        if mediaType:
            key += '?mediaType={}'.format((utils.searchType(mediaType)))
    ...
    def createToken(self, type='delegation', scope='all'):
        ...
        if not self._token:
            ...
            return None
        q = self.query('/security/token?type={}&scope={}'.format((type), (scope)))
    ...
    def systemAccount(self, accountID):
        ...
        try:
            ...
            raise NotFound('Unknown account with accountID={}'.format((accountID)))
    ...
    def systemDevice(self, deviceID):
        ...
        try:
            ...
            raise NotFound('Unknown device with deviceID={}'.format((deviceID)))
    ...
    def clients(self):
        ...
        for elem in self.query('/clients'):
            ...
            baseurl = "http://{}:{}".format((elem.attrib['host']), (port))
    ...
    def client(self, name):
        ...
        raise NotFound('Unknown client name: {}'.format((name)))
    ...
    def runButlerTask(self, task):
        ...
        if task not in validTasks:
            raise BadRequest(
                'Invalid butler task: {}. Available tasks are: {}'.format((task), (validTasks))
            )
        self.query('/butler/{}'.format((task)), method=self._session.post)
        return self
    ...
    def checkForUpdate(self, force=True, download=False):
        ...
        """
        part = '/updater/check?download={}'.format((1 if download else 0))
        if force:
    ...
    def history(self, maxresults=None, mindate=None, ratingKey=None, accountID=None, librarySectionID=None):
        ...
        key = '/status/sessions/history/all{}'.format((utils.joinArgs(args)))
    ...
    def playlists(self, playlistType=None, sectionId=None, title=None, sort=None, **kwargs):
        ...
        key = '/playlists{}'.format((utils.joinArgs(args)))
    ...
    def optimizedItems(self, removeAll=None):
        ...
        if removeAll is True:
            ...
            return self.fetchItems('{}/items'.format((backgroundProcessing.key)), cls=Optimized)
    ...
    def optimizedItem(self, optimizedID):
        ...
        backgroundProcessing = self.fetchItem('/playlists?type=42')
        return self.fetchItem('{}/items/{}/items'.format((backgroundProcessing.key), (optimizedID)))
    ...
    def search(self, query, mediatype=None, limit=None, sectionId=None):
        ...
        key = '/hubs/search?{}'.format((urlencode(params)))
    ...
    def transcodeImage(self, imageUrl, height, width,
        ...
        key = '/photo/:/transcode{}'.format((utils.joinArgs(params)))
    ...
    def url(self, key, includeToken=None):
        ...
        if self._token and (includeToken or self._showSecrets):
            ...
            return '{}{}{}X-Plex-Token={}'.format((self._baseurl), (key), (delim), (self._token))
        return '{}{}'.format((self._baseurl), (key))
    ...
    def _allowMediaDeletion(self, toggle=False):
        ...
        return self.query('/:/prefs?allowMediaDeletion={}'.format((value)), self._session.put)
    ...
    def bandwidth(self, timespan=None, **kwargs):
        ...
        if timespan is None:
            ...
            try:
                ...
                raise BadRequest("Invalid timespan specified: {}. "
                                 "Available timespans: {}".format((timespan), (', '.join(timespans.keys()))))

        filters = {'accountID', 'at', 'at<', 'at>', 'bytes', 'bytes<', 'bytes>', 'deviceID', 'lan'}

        for key, value in kwargs.items():
            if key not in filters:
                raise BadRequest('Unknown filter: {}={}'.format((key), (value)))
            if key.startswith('at'):
                try:
                    value = utils.cast(int, timestamp(value))
                except AttributeError:
                    raise BadRequest('Time frame filter must be a datetime object: {}={}'.format((key), (value)))
        ...
        key = '/statistics/bandwidth?{}'.format((urlencode(params)))
    ...
    def _buildWebURL(self, base=None, endpoint=None, **kwargs):
        ...
        if endpoint:
            return '{}#!/server/{}/{}{}'.format((base), (self.machineIdentifier), (endpoint), (utils.joinArgs(kwargs)))
        else:
            return '{}#!/media/{}/com.plexapp.plugins.library{}'.format((base), (self.machineIdentifier), (utils.joinArgs(kwargs)))
    ...
    def getWebURL(self, base=None, playlistTab=None):
        ...
        if playlistTab is not None:
            params = {'source': 'playlists', 'pivot': 'playlists.{}'.format((playlistTab))}
        else:
...
class SystemDevice(PlexObject):
    ...
    def _loadData(self, data):
        ...
        self.id = utils.cast(int, data.attrib.get('id'))
        self.key = '/devices/{}'.format((self.id))
        self.name = data.attrib.get('name')
        self.platform = data.attrib.get('platform')
...
class StatisticsResources(PlexObject):
    ...
    def __repr__(self):
        return "<{}>".format((':'.join([p for p in [self.__class__.__name__, self._clean(int(timestamp(self.at)))] if p])))
...
class Identity(PlexObject):
    ...
    def __repr__(self):
        return "<{}:{}>".format((self.__class__.__name__), (self.machineIdentifier))
