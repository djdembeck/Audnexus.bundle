...
class Library(PlexObject):
    ...
    def sectionByID(self, sectionID):
        ...
        try:
            ...
            raise NotFound('Invalid library sectionID: {}'.format((sectionID)))
    ...
    def hubs(self, sectionID=None, identifier=None, **kwargs):
        ...
        key = '/hubs{}'.format((utils.joinArgs(kwargs)))
    ...
    def search(self, title=None, libtype=None, **kwargs):
        ...
        for attr, value in kwargs.items():
            args[attr] = value
        key = '/library/all{}'.format((utils.joinArgs(args)))
        return self.fetchItems(key)
    ...
    def add(self, name='', type='', agent='', scanner='', location='', language='en', *args, **kwargs):
        ...
        for path in location:
            if not self._server.isBrowsable(path):
                raise BadRequest('Path: {} does not exist.'.format((path)))
            locations.append(('location', path))

        part = ('/library/sections?name={}&type={}&agent={}'
                '&scanner={}&language={}&{}'.format((quote_plus(name)), (type), (agent), (quote_plus(scanner)), (language), (urlencode(locations, doseq=True))))
        if kwargs:
    ...
    def tags(self, tag):
        ...
        data = self._server.query('/library/tags?type={}'.format((tagType)))
...
class LibrarySection(PlexObject):
        ...
        if callable(value) and 'Mixin' in _qualname:
            if not isinstance(self._edits, dict):
                ...
                raise AttributeError(
                    "Batch multi-editing '{}' object has no attribute '{}'".format(
                        (self._edits['items'][0].__class__.__name__), (attr))
                )
        return value
    ...
    def totalViewSize(self, libtype=None, includeCollections=True):
        ...
        if libtype is not None:
            if libtype == 'photo':
                ...
                args['type'] = utils.searchType(libtype)
        part = '/library/sections/{}/all{}'.format((self.key), (utils.joinArgs(args)))
    ...
    def delete(self):
        ...
        try:
            return self._server.query('/library/sections/{}'.format((self.key)), method=self._server._session.delete)
        except BadRequest:  # pragma: no cover
            msg = 'Failed to delete library {}'.format((self.key))
    ...
    def edit(self, agent=None, **kwargs):
        ...
        part = '/library/sections/{}?agent={}&{}'.format((self.key), (agent), (urlencode(params, doseq=True)))
    ...
    def removeLocations(self, location):
        ...
        for path in location:
            if path in locations:
                ...
                raise BadRequest('Path: {} does not exist in the library.'.format((location)))
        if len(locations) == 0:
    ...
    def get(self, title, **kwargs):
        ...
        try:
            ...
            msg = "Unable to find item with title '{}'".format((title))
            if kwargs:
                msg += " and kwargs {}".format((kwargs))
            raise NotFound(msg)
    ...
    def getGuid(self, guid):
        ...
        try:
            ...
            raise NotFound("Guid '{}' is not found in the library".format((guid)))
    ...
    def folders(self):
        ...
        """
        key = '/library/sections/{}/folder'.format((self.key))
    ...
    def managedHubs(self):
        ...
        """
        key = '/hubs/sections/{}/manage'.format((self.key))
    ...
    def hubs(self):
        ...
        """
        key = '/hubs/sections/{}?includeStations=1'.format((self.key))
        return self.fetchItems(key)
    ...
    def settings(self):
        ...
        key = '/library/sections/{}/prefs'.format((self.key))
    ...
    def editAdvanced(self, **kwargs):
        ...
        for settingID, value in kwargs.items():
            try:
                ...
                raise NotFound('{} not found in {}'.format((value), (list(idEnums.keys()))))
            if value in enums:
                data[key.format(settingID)] = value
            else:
                raise NotFound('{} not found in {}'.format((value), (enums)))
    ...
    def _lockUnlockAllField(self, field, libtype=None, locked=True):
        ...
        args = {
            ...
            '{}.locked'.format((field)): int(locked)
        }
        key = '/library/sections/{}/all{}'.format((self.key), (utils.joinArgs(args)))
    ...
    def timeline(self):
        ...
        key = '/library/sections/{}/timeline'.format((self.key))
        data = self._server.query(key)
        return LibraryTimeline(self, data)

    def onDeck(self):
        """ Returns a list of media items on deck from this library section. """
        key = '/library/sections/{}/onDeck'.format((self.key))
        return self.fetchItems(key)

    def continueWatching(self):
        """ Return a list of media items in the library's Continue Watching hub. """
        key = '/hubs/sections/{}/continueWatching/items'.format((self.key))
        return self.fetchItems(key)
    ...
    def firstCharacter(self):
        key = '/library/sections/{}/firstCharacter'.format((self.key))
    ...
    def analyze(self):
        ...
        """
        key = '/library/sections/{}/analyze'.format((self.key))
        self._server.query(key, method=self._server._session.put)
        return self

    def emptyTrash(self):
        """ If a section has items in the Trash, use this option to empty the Trash. """
        key = '/library/sections/{}/emptyTrash'.format((self.key))
        self._server.query(key, method=self._server._session.put)
        return self
    ...
    def update(self, path=None):
        ...
        """
        key = '/library/sections/{}/refresh'.format((self.key))
        if path is not None:
            key += '?path={}'.format((quote_plus(path)))
    ...
    def refresh(self):
        """ Forces a download of fresh media information from the internet.
            This can take a long time. Any locked fields are not modified.
        """
        key = '/library/sections/{}/refresh?force=1'.format((self.key))
        self._server.query(key)
        return self
    ...
    def deleteMediaPreviews(self):
        ...
        """
        key = '/library/sections/{}/indexes'.format((self.key))
        self._server.query(key, method=self._server._session.delete)
        return self
    ...
    def getFilterType(self, libtype=None):
        ...
        try:
            ...
            raise NotFound('Unknown libtype "{}" for this library. '
                           'Available libtypes: {}'.format((libtype), (availableLibtypes)))
    ...
    def getFieldType(self, fieldType):
        ...
        try:
            ...
            raise NotFound('Unknown field type "{}" for this library. '
                           'Available field types: {}'.format((fieldType), (availableFieldTypes)))
    ...
    def listFilterChoices(self, field, libtype=None):
        ...
        if isinstance(field, str):
            ...
            if not match:
                raise BadRequest('Invalid filter field: {}'.format((field)))
            ...
            try:
                ...
                raise NotFound('Unknown filter field "{}" for libtype "{}". '
                               'Available filters: {}'.format((field), (libtype), (availableFilters)))
    ...
    def _validateFilterField(self, field, values, libtype=None):
        ...
        try:
            ...
            for filterType in reversed(self.filterTypes()):
                ...
                raise NotFound('Unknown filter field "{}" for libtype "{}". '
                               'Available filter fields: {}'.format((field), (libtype), (availableFields)))
    ...
    def _validateFieldOperator(self, filterField, operator):
        ...
        try:
            ...
            raise NotFound('Unknown operator "{}" for filter field "{}". '
                           'Available operators: {}'.format((operator), (filterField.key), (availableOperators)))
    ...
    def _validateFieldValue(self, filterField, values, libtype=None):
        ...
        try:
            ...
            raise BadRequest('Invalid value "{}" for filter field "{}", '
                             'value should be type {}'.format((value), (filterField.key), (fieldType.type)))
    ...
    def _validateSortField(self, sort, libtype=None):
        ...
        if isinstance(sort, FilteringSort):
            return '{}.{}:{}'.format((libtype or self.TYPE), (sort.key), (sort.defaultDirection))

        match = re.match(r'(?:([a-zA-Z]*)\.)?([a-zA-Z]+):?([a-zA-Z]*)', sort.strip())
        if not match:
            raise BadRequest('Invalid filter sort: {}'.format((sort)))
        ...
        try:
            ...
            raise NotFound('Unknown sort field "{}" for libtype "{}". '
                           'Available sort fields: {}'.format((sortField), (libtype), (availableSorts)))

        sortField = libtype + '.' + filterSort.key

        availableDirections = ['', 'asc', 'desc', 'nullsLast']
        if sortDir not in availableDirections:
            raise NotFound('Unknown sort direction "{}". Available sort directions: {}'.format(
                (sortDir), (availableDirections)))

        return '{}:{}'.format((sortField), (sortDir)) if sortDir else sortField
        ...
        for field, values in filters.items():
            if field.lower() in {'and', 'or'}:
                ...
                for value in values:
                    ...
                    validatedFilters.append('{}=1'.format((field.lower())))
    ...
    def _buildSearchKey(self, title=None, sort=None, libtype=None, limit=None, filters=None, returnKwargs=False, **kwargs):
        ...
        key = '/library/sections/{}/all?{}'.format((self.key), (params))
    ...
    def sync(self, policy, mediaSettings, client=None, clientId=None, title=None, sort=None, libtype=None,
        ...
        sync_item.location = 'library://{}/directory/{}'.format((self.uuid), (quote_plus(key)))
    ...
    def collection(self, title):
        ...
        try:
            ...
            raise NotFound('Unable to find collection with title "{}".'.format((title)))
    ...
    def playlist(self, title):
        ...
        try:
            ...
            raise NotFound('Unable to find playlist with title "{}".'.format((title)))
    ...
    def _validateItems(self, items):
        ...
        for item in items:
            if item.librarySectionID != self.key:
                raise BadRequest('{} is not from this library.'.format((item.title)))
            elif item.type != itemType:
                raise BadRequest('Cannot mix items of different type: {} and {}'.format((itemType), (item.type)))
    ...
    def common(self, items):
        ...
        part = '/library/sections/{}/common{}'.format((self.key), (utils.joinArgs(params)))
    ...
    def _edit(self, items=None, **kwargs):
        ...
        part = '/library/sections/{}/all{}'.format((self.key), (utils.joinArgs(kwargs)))
        self._server.query(part, method=self._server._session.put)
        return self
...
class MusicSection(LibrarySection, ArtistEditMixins, AlbumEditMixins, TrackEditMixins):
    ...
    def albums(self):
        ...
        key = '/library/sections/{}/albums'.format((self.key))
        return self.fetchItems(key)
...
class LibraryMediaTag(PlexObject):
    ...
    def items(self, *args, **kwargs):
        ...
        if not self.key:
            raise BadRequest('Key is not defined for this tag: {}'.format((self.tag)))
...
class FilteringType(PlexObject):
    ...
    def __repr__(self):
        ...
        return "<{}>".format((':'.join([p for p in [self.__class__.__name__, _type] if p])))
    ...
    def _manualFilters(self):
        ...
        for filterTag, filterType, filterTitle in additionalFilters:
            filterKey = '/library/sections/{}/{}?type={}'.format((self._librarySectionID),
                                                                 (filterTag), (utils.searchType(self.type)))
            filterXML = (
                '<Filter filter="{}" '
                'filterType="{}" '
                'key="{}" '
                'title="{}" '
                'type="filter" />'.format((filterTag), (filterType), (filterKey), (filterTitle))
            )
    ...
    def _manualSorts(self):
        ...
        additionalSorts = [
            ('guid', 'asc', 'Guid'),
            ('id', 'asc', 'Rating Key'),
            ('index', 'asc', '{} Number'.format((self.type.capitalize()))),
            ('summary', 'asc', 'Summary'),
            ('tagline', 'asc', 'Tagline'),
        ...
        for sortField, sortDir, sortTitle in additionalSorts:
            sortXML = (
                ...
                'descKey="{}:desc" '
                'key="{}" '
                'title="{}" />'.format((sortDir), (sortField), (sortField), (sortTitle))
            )
...
class ManagedHub(PlexObject):
    ...
    def reload(self):
        ...
        key = '/hubs/sections/{}/manage'.format((self.librarySectionID))
    ...
    def move(self, after=None):
        ...
        key = '/hubs/sections/{}/manage/{}/move'.format((self.librarySectionID), (self.identifier))
        if after:
            key = '{}?after={}'.format((key), (after.identifier))
        self._server.query(key, method=self._server._session.put)
    ...
    def remove(self):
        ...
        if not self.deletable:
            raise BadRequest('{} managed hub cannot be removed'.format((self.title)) % self.title)
        key = '/hubs/sections/{}/manage/{}'.format((self.librarySectionID), (self.identifier))
        self._server.query(key, method=self._server._session.delete)
