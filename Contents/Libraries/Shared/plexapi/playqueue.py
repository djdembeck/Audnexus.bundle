...
class PlayQueue(PlexObject):
    ...
    def __iter__(self):
        for _plexapi_backport_yield in self.items:
            yield _plexapi_backport_yield
    ...
    def getQueueItem(self, item):
        ...
        if len(matches) == 1:
            ...
            raise BadRequest(
                "{} occurs multiple times in this PlayQueue, provide exact item".format((item))
            )
        else:
            raise BadRequest("{} not valid for this PlayQueue".format((item)))
    ...
    def get(
        ...
        path = "/playQueues/{}{}".format((playQueueID), (utils.joinArgs(args)))
    ...
    def create(
        ...
        if isinstance(items, list):
            ...
            uri_args = quote_plus("/library/metadata/{}".format((item_keys)))
            args["uri"] = "library:///directory/{}".format((uri_args))
            ...
            args["uri"] = "server://{}/{}{}".format((server.machineIdentifier), (server.library.identifier), (items.key))

        if startItem:
            args["key"] = startItem.key

        path = "/playQueues{}".format((utils.joinArgs(args)))
    ...
    def fromStationKey(cls, server, key):
        ...
        args = {
            "type": "audio",
            "uri": "server://{}/{}{}".format((server.machineIdentifier), (server.library.identifier), (key))
        }
        path = "/playQueues{}".format((utils.joinArgs(args)))
        data = server.query(path, method=server._session.post)
        c = cls(server, data, initpath=path)
    ...
    def addItem(self, item, playNext=False, refresh=True):
        ...
        if item.type == "playlist":
            ...
            args["uri"] = "library://{}/item{}".format((uuid), (item.key))

        if playNext:
            args["next"] = 1

        path = "/playQueues/{}{}".format((self.playQueueID), (utils.joinArgs(args)))
    ...
    def moveItem(self, item, after=None, refresh=True):
        ...
        path = "/playQueues/{}/items/{}/move{}".format((self.playQueueID), (item.playQueueItemID), (utils.joinArgs(args)))
        data = self._server.query(path, method=self._server._session.put)
        self._loadData(data)
        return self
    ...
    def removeItem(self, item, refresh=True):
        ...
        path = "/playQueues/{}/items/{}".format((self.playQueueID), (item.playQueueItemID))
    ...
    def clear(self):
        ...
        path = "/playQueues/{}/items".format((self.playQueueID))
        data = self._server.query(path, method=self._server._session.delete)
        self._loadData(data)
        return self
    ...
    def refresh(self):
        ...
        path = "/playQueues/{}".format((self.playQueueID))
